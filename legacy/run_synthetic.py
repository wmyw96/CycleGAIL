...
from legacy.CycleGAIL import CycleGAIL
from utils import greedy_reject_sampling, \
    spindrive3d_generate_random_trajectories
...
def generate_auxiliary_samples(id, nitems, radius, alpha, dt, rg):
    ...
    for i in range(nitems):
        state, action = \
            spindrive3d_generate_random_trajectories(150, radius,
                                                     alpha, dt, rg)
        demos.add_demo(state, action)
        print(name + ": trajectory %d finished!" % i)
    demos.save(name)
...
parser.add_argument('--dhid', dest='dhid', type=int, default=10,
...
parser.add_argument('--dlayer', dest='dlayer', type=int, default=2,
                    help='hidden size of the discriminator D')
...
parser.add_argument('--aneal_epoches', dest='aneal_epoches', type=int,
                    default=500, help='aneal the lr after # of epoches')
parser.add_argument('--aneal_rate', dest='aneal_rate', type=float,
                    default=1.0, help='aneal rates')
...
parser.add_argument('--metric', dest='metric', default='L1',
                    help='L1, or L2')
...
parser.add_argument('--clip', dest='clip', type=float, default=0.02,
                    help='clip value')
parser.add_argument('--ntraj', dest='ntraj', type=int, default=20,
                    help='# of trajctories in the data set')
parser.add_argument('--n_c', dest='n_c', type=int, default=10,
                    help='n_critic')
...
if args.mode == 'train':
    ...
    with tf.Session() as sess:
        model = CycleGAIL(args.name, sess, args.clip, 5, enva, envb, init_align,
...
'''
Recommended command:
python3 run_synthetic.py --lr 0.0001 --dhid 30 --ghid 10 --fhid 10 --loss_metric L2 --epoch 5000
'''